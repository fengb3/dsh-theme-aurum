...
| P9 | §5 | 会话流尾部节点:turn-tail/compress/retry/err/max-tokens + md 装饰 + 头像 + 列宽712(sh-head/tabs、reasoning、typing 残留) | htm 恒等映射 + conversation.chat.node 补 key | ◐ |
...
- ◐ 滚动条:经 `--dsw-alias-scrollbar-*` token 换色;原型还有 10px 宽 / thumb 26% 透明度 /
  hover 金的固定几何,需按原型微调(P9 一并处理)。
- ◐ 栏头同轴机制:原型 `--rail-inset:12 + --rail-head:58` 保证侧栏品牌行/主区标题中心线同轴。
  侧栏 logoRow 58px 已做;**主区 sh-head(高 70=12+58)完全未做**,依赖 P9。
- ⬜ 详情栏去留:原型已**取消右栏**(§7 注释「原详情栏内容并入卡片」,工具卡点击就地展开);
  当前仍保留官方详情栏并卡片化。需在 P11 决策:隐藏右栏+详情并入工具卡,或保留(记入「不适用」)。

### §4 侧栏(P8 主体已对齐,以下为残差 → P8c)

- ⬜ 折叠细条宽度:原型 `.app.no-sb .sidebar{width:56px;border-radius:17px}`(rail 钮 40px
  居中、左右各 8 留白);当前折叠卡实测 40px、按钮贴边。需放宽到 56。
- ⬜ 折叠过渡:原型展开 264⇄56 宽度过渡(.42s cubic-bezier(.22,.8,.26,1))且主内容⇄细条
  **交叉淡切**(sb-main 淡出后 sb-rail 延迟淡入);当前仅 radius 过渡,宽度交给官方动画。
- ⬜ rail-logo 交叉淡切:原型折叠态 logo 悬停时「鲸鱼 ⇄ 展开面板图标」透明度+缩放互切
  (`.rail-logo .rl-whale/.rl-panel`),点击展开;当前折叠态 logo 行为=官方。
- ⬜ 细条按钮组:原型 rail = logo / 新建(金 tint)/ 搜索 + flex + 设置/主题;当前 AuBrowser
  折叠分支只渲染一个搜索钮(点击仅展开侧栏,原型行为=展开后 300ms 聚焦搜索框)。
- ⬜ 会话行拖拽排序:原型 `draggable` + `drop-before/after` 金线落点 + `.dragging` 半透明;
  当前无拖拽。
- ◐ 行操作形态:原型=悬停 ··· 弹**浮动菜单**(`.menu/.mi`:置顶 ⌘P / 重命名 F2 / 导出会话 /
  分隔线 / 删除 danger,带 mk 快捷键列);当前=行内展开操作条(重命名/分支/归档)。
  DSH 侧有实义 fork/archive API 而无置顶/导出 —— 保留现交互或对齐菜单形态,二选一。
- ◐ 视图选项:原型点「视图」弹菜单(按最近活动排序/按名称排序/平铺会话列表/导出全部目录);
  当前单击直接切换平铺,排序与导出缺失。
- ◐ 品牌字标:原型 DSH wordmark SVG 182×24,尾部带金色 badge(反白小字);当前用官方
  brand(高 22px),badge 形态不同。

### §5 会话流(→ P9)

- ✅ 用户气泡(右对齐、22px 圆角、金渐变 tint、≤525px/82%)、◈ 上下文行、图片附件。
- ◐ ◈ 行 token 标注:原型有 `.in-tok`(「已注入上下文 · 2.1k tokens」);当前 AuContext 仅文本。
- ⬜ sh-head 主区标题栏:原型为浮动渐变头(底部渐隐)、DISPLAY 18.5px 标题、mono `.sh-meta`
  副行、右侧 **tabs 胶囊(会话/轨迹,金色 on 态)**、≤1024 显示 burger。当前未动官方头部。
- ⬜ assistant 节:原型 28px 圆形金鲸鱼头像(`.a-avatar`) + **reasoning 推理折叠段**
  (mono 头「已推理 8.4s」/ serif italic 正文/虚线分隔) + `.a-actions` 悬停操作(复制/分支)。
  当前 assistant 回复完全走官方渲染。
- ⬜ md 装饰:原型衬线正文 15px/1.95、`li::before` ◆ 金点、`code` 金色 mono pill(surface-2
  底)、`.hl` 金强、`text-wrap:pretty`。当前 FONT_TOKENS 只换字体族,装饰规则未加。
- ⬜ compress 压缩标记(◈ 折叠头 + 虚线摘要卡)、row-err(红 tint 行内错误 + err pill)、
  row-retry(自动重试行,成功 b 绿)、turn-tail(细线 + mono 耗时统计)、typing 三金点 bob。
- ⬜ 入场节奏:原型逐 node 阶梯 rise(每节点 +75ms);当前整个 session 容器一次性 rise。
...
### P8c · 侧栏残差(原型 §4)
- 折叠细条 40→56px(rail 钮 40 居中、左右 8 留白),展开⇄折叠加宽度过渡 + 主内容⇄细条交叉淡切;
- rail-logo 悬停「鲸鱼⇄展开面板」交叉淡切,点击展开;细条补新建/设置/主题钮,搜索钮=展开+聚焦;
- 会话行拖拽排序(drop 金线落点);
- 决策项:行操作对齐原型浮动菜单(置顶/重命名/导出/删除)还是保留现行内操作条(分支/归档);
  视图选项补排序(最近活动/名称)与导出菜单。
- 门禁:折叠态卡 56px、零溢出;过渡全程无内容跳变;`verify-gate.js` 三态复跑。

### P9 · 会话流非工具节点(原型 §5)—— 恒等映射流水首个试点 ◐
...
**残留**:sh-head 浮动渐变头 + tabs 胶囊(动官方头部,工作量集中);reasoning 折叠段
(官方有 ReasoningRow 组件,需摸清数据形状再映射);typing 三金点;◈ 行 in-tok 标注。
